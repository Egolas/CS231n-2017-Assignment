...
class KNearestNeighbor(object):
  ...
  def compute_distances_no_loops(self, X):
    ...
    dists = np.sqrt(-2*M + np.matrix(A).T + B)
    return dists
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      #########################################################################
      closest_y = dists[i]
      closest_y = closest_y.argsort()
      closest_y = closest_y[:k]
      # print(closest_y.shape)
      closest_y_label = self.y_train[closest_y]
      # print(closest_y_label.shape)
      pass
      #########################################################################
      # TODO:                                                                 #
      ...
      label = {}
      for d in closest_y_label:
        if d not in label:
          label[d] = 1
        else:
          label[d] += 1
      y_pred[i] = max(label.items(), key = lambda x: x[1])[0]
      pass
      #########################################################################
      #                           END OF YOUR CODE                            # 
      #########################################################################
